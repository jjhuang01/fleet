...
- **`Cmd+V` pastes a screenshot into a pane as a file path** - a clipboard with a picture and no text is written to `$TMPDIR/fleet-paste/image-<ms>.png` and that path is put on the pane's command line, which is what Codex, Claude Code and `pi` accept and what Otty does (it writes `$TMPDIR/otty-paste/`). Text still wins when the clipboard holds both, a second paste in the same millisecond gets its own file rather than overwriting the one a prompt already names, and plain `Ctrl+V` is left alone for the CLIs that read the clipboard themselves.
A pane on the far side of ssh gets the picture itself: the file is uploaded into the remote shell's own directory and the prompt gets the remote path, because a path in this machine's temp directory is one the far side cannot open. The file is written `0600` in a `0700` directory rather than left world-readable in a shared `/tmp`, and pictures a week old are swept on start. Measured in a live pane, clipboard holding a region screenshot: `Cmd+V` left `/var/folders/.../T/fleet-paste/image-1789333357637.png` on the input line, and the file on disk is `-rw-------`. [docs/otty-parity-notes.md](docs/otty-parity-notes.md)
- **Typing with an IME no longer prints the text twice** - taking a candidate with Enter could put `codexcodex` in the pane. The guard told a real duplicate from the user retyping the same text by counting keystrokes, but one Enter looks exactly like any other and the flushing Enter's `\r` arrives *between* the two copies of the text, so no counting order could separate them. An exact repeat inside the window is now dropped, and the episode ends only on something a duplicate cannot be - a new composition, a paste, or the window expiring. Retyping the same text on purpose is unaffected: with an IME it is a new composition, and without one it arrives as single characters rather than as one chunk equal to the whole commit.
The text it compares against now comes from the terminal's own textarea rather than from `CompositionEvent.data`, because Chromium leaves that field empty - `null` or `''`, depending on the IME - and the guard used to forget the composition when it saw that. That is what let the duplicate back in after the first fix shipped; the range xterm re-reads is the string it re-sends, so the repeat is recognised whatever the event carries.
- **Reading back through a pane that is printing no longer gets pulled to the bottom** - an agent streaming into a pane made scrollback unreadable: the "scrolled up" strip appeared and the next chunk of output took it away. Whether a pane should follow was derived from its position (`viewportY >= baseY - 2`), so a view one notch up still counted as being at the bottom, and the wheel handler's deferred check ran late enough during fast output to see the bottom again. Following is now a state only a gesture changes, which is the rule the ecosystem settled on for the same problem in chat UIs: an upward gesture escapes, a downward one re-locks once the view is back within the couple of rows of lag that fast output earns, and output, `scrollToBottom` and resizes may only re-lock at the exact bottom. `Shift+PageUp/PageDown` - the one keyboard gesture xterm itself scrolls on - take the same path as a wheel notch. [docs/learnings/2026-09-14-follow-intent-is-a-user-gesture.md](docs/learnings/2026-09-14-follow-intent-is-a-user-gesture.md)
- **A pane can be dragged somewhere else instead of closed and retyped** - dropping a pane on another pane's edge re-docks it there, dropping it on a sidebar tab folds that pane into that tab, and dropping it on the empty sidebar below the tabs pulls it back out into a tab of its own, so a merge is reversible rather than a one-way door. The PTY is never rebuilt, so a running agent keeps its session, its scrollback and its working directory through any of those moves, and `Cmd+Alt+Arrow` does the same thing from the keyboard. Neither Otty branch has pane dragging or tab merging - [docs/pane-layout-review.md](docs/pane-layout-review.md) checks both against their source and says which capability came from where.
- **Two dividers that meet at an intersection move as one** - a 2x2 grid needed to trade a corner, and dragging one divider at a time cannot express that: the crossing point now moves the whole row and the whole column together, keeping the panes it cuts collinear. The strip that answers the pointer grew to 12px, matching the leeway iced gives Otty's own splitters, while the seam it draws stays 6px so the gutters between panes read the same. A divider also shows itself on hover and on focus rather than being invisible until it is grabbed, because a control that only appears once you have already found it is not discoverable.
- **Splitting equalizes, and `Cmd+Shift+B` re-balances** - `Cmd+D` used to hand the new pane a fixed fraction of its parent, so a second split next to a first produced three panes of three different widths. A split now re-computes the panes it divides to equal size, which is what Otty's `pane_balance.rs` does after the same key, and the explicit balance command re-equalizes a tab that dragging has left lopsided. `lib/pane-balance.ts` reproduces the semantics from Otty's source; no code crossed over, since Otty is Rust and iced and this is TypeScript.

- **The interface speaks Simplified Chinese** - `Settings > General > Language` switches the app between English and Simplified Chinese, and the default (`Follow system`) reads the OS locale, so a Chinese Mac opens in Chinese without configuring anything.
Everything a user reads is covered: the shell and its lists, every Settings page, the agent pane and its cards, the SSH browser, the notes, `.env`, session and tool dialogs, and the pieces the renderer cannot reach - the terminal's right-click menu, the window title and desktop notifications, which the main process translates against the same preference.
What stays English is listed in [docs/i18n.md](docs/i18n.md): third-party and runtime text (model output, MCP and tool descriptions, SFTP errors), identifier-shaped strings (model ids, JSON keys, env vars, commands), and the release notes, which are the project's own record rather than interface copy.
...
- **One catalogue, typed against English** - every string lives in `src/shared/i18n/`, split into per-surface files so two people can translate two screens without editing one 1700-line list. The English catalogue defines `MessageKey`; each translation is typed `Record<MessageKey, string>`, so a screen that is half-translated does not compile. Counted sentences take an `...One`/`...Other` pair rather than an appended "s", because that rule is English's and not the language file's.
- **Timestamps follow the language too** - the duplicated `toLocaleDateString()`/relative-time helpers scattered across the sidebar, sessions, SSH and shell-env views now go through one pair of formatters that take the locale.
...
- **The last line before a fast exit survives** - the PTY flush runs on a 16ms tick, so a command that exited between two ticks had everything it printed since the last one dropped along with its callbacks: the end of a compiler's error, or a test run's summary. The tail is now flushed before the exit is reported, and the regression test fails without that flush.
- **Overlapping MCP connects stop stacking up** - a reload, a reconnect or a sign-in that overlapped another one let the displaced client go unclosed, and piped a stdio server's stderr that nothing read, which accumulates child processes and can hang a tool call. A superseded connect now closes what it replaced, and stderr is discarded rather than piped.
- **Quitting waits for the children, and closing releases per-pane state** - Electron does not await a promise returned from `will-quit`, so only the first cleanup step ran and the remaining MCP child processes were reparented; closing a window also left the cwd poller, activity tracking and notification state working against a pane that no longer existed. Quit now waits for children up to a bound and then exits, and both the window-close and the natural-exit paths release per-pane state.
...
- **A pane that printed and exited in the background keeps its last lines** - a hidden pane defers its terminal writes to a 250 ms timer, so its newest output can still be sitting in that buffer when its tab is closed and snapshotted for the undo. The snapshot now lets the buffer land before it reads, so reopening the tab shows where the output actually ended instead of a version that stops early. The read-only pane tail takes the same step.
- **Copilot's hook socket closes after every event it has no answer for** - the listener runs with `allowHalfOpen`, so a hook that sent its event and exited left the server's half of the connection open, which is one file descriptor per event for the life of the process. An event that is not being held for a permission answer now closes its socket; the ones that are held still wait for the answer they are owed.
- **The PreToolUse pairing cannot grow for the life of a session** - the store caches a `tool_use_id` under a key made of the tool plus its full input, and an entry survives for every PreToolUse whose permission event never arrived, which is most of them. The cache is capped at 500 keys and drops the oldest.
- **Backpressure now ends where the output does** - a pane whose output outran the renderer was paused, but the resume came from a 250 ms timer on this side rather than from the renderer saying it had caught up, and the acknowledgement carried no size, so a renderer that was behind simply stopped seeing backpressure after the first tick. The renderer now acknowledges the bytes it has actually handed to xterm, and the pane resumes when every byte sent while paused has been acknowledged. The timer stays as an escape from a renderer that never answers, at 2 s rather than 250 ms, so it can clear the renderer's own slowest normal path (a hidden pane flushing every 250 ms) instead of second-guessing it.
- **A pane's colours no longer depend on what started Fleet** - launching from a shell inside another terminal, or from an agent CLI, left that process's `NO_COLOR`, `TERM_PROGRAM` and `OTTY_*` bookkeeping in the environment of every pane. `NO_COLOR` is the one that shows: every colour-aware CLI honours it, so Claude Code, Codex, `git` and `bat` all came up in a single colour that no theme in Settings could undo. The login shell is now asked for the user's environment with the launcher's terminal identity removed, and panes are stamped with Fleet's own (`TERM_PROGRAM=Fleet`, `COLORTERM=truecolor`; `TERM` already comes from the pty). A `NO_COLOR` the user sets in their own rc still comes back from the shell and is still honoured, VS Code's `VSCODE_GIT_ASKPASS_*` credentials plumbing is left alone (only `TERM_PROGRAM=vscode`, the identity, is dropped), panes carry Fleet's own identity from the first one created rather than only after the login shell answers, and a pane waits for that answer before it copies the environment at all. Measured: a pane in an app started with `NO_COLOR=1` rendered SGR 31/32/34/38;2 as one colour before the change and as `rgb(255,92,87)` / `rgb(90,247,142)` / `rgb(87,199,255)` / `rgb(255,128,0)` after it. [docs/otty-parity-notes.md](docs/otty-parity-notes.md) compares the rest of the rendering surface against the installed Otty.

- **Dialogs are dialogs to a screen reader, and Tab stays inside them** - the shared overlay shell drew a backdrop and handled Escape, but carried no `role="dialog"` and no `aria-modal`, so assistive technology read a modal as a plain div; and because the browser does not enforce `aria-modal`, Tab walked straight out of the dialog and into the terminals behind it. The panel is now a labelled modal: it is named after the heading it already shows (the eight panels that draw no heading name themselves), focus moves into it on open and returns to whatever had it on close, and Tab wraps at both ends. Three ways out are closed with it: a control taken out of the Tab order with `tabindex="-1"` no longer counts as a stop, a dialog whose only control is disabled holds Tab on the panel instead of falling through, and a panel that focuses its own field on mount no longer takes the returned-to control down with it when it closes.
...
- **Release notes read as notes** - they were rendered as literal text, so every `-` and `**` in this file showed up as punctuation. See the [changelog](https://github.com/jjhuang01/fleet/blob/main/CHANGELOG.md) for the full history.
