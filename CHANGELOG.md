...
- **`Cmd+V` pastes a screenshot into a pane as a file path** - a clipboard with a picture and no text is written to `$TMPDIR/fleet-paste/image-<ms>.png` and that path is put on the pane's command line, which is what Codex, Claude Code and `pi` accept and what Otty does (it writes `$TMPDIR/otty-paste/`). Text still wins when the clipboard holds both, a second paste in the same millisecond gets its own file rather than overwriting the one a prompt already names, and plain `Ctrl+V` is left alone for the CLIs that read the clipboard themselves. Measured in a live pane: `Cmd+V` left `/var/folders/.../T/fleet-paste/image-1789328612168.png` on the input line while the 2400x1600 PNG landed on disk. [docs/otty-parity-notes.md](docs/otty-parity-notes.md)
- **Wheel scrolling eases instead of jumping** - a notch used to land the whole distance in one frame. `smoothScrollDuration: 125` spreads it over the animation, which is the value VS Code's terminal uses and the closest xterm's DOM renderer gets to Otty's `terminal-scroll-smooth`: that renderer draws whole rows and has no sub-row offset, so the steps stay row-sized and only their timing changes. Measured against the same xterm 6.0.0 build, one notch went `376 → 373` in a single step at `0` and `376 → 375 → 374 → 373` over 57ms at `125`, with p95 frame pacing of 17.7ms through a burst of 20 notches. [docs/otty-parity-notes.md](docs/otty-parity-notes.md)
