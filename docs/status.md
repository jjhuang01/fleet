...
| Terminal colour              | `f720fac8`, `5ce5dd73`                                                              | A Fleet started from inside another terminal, or from an agent CLI, used to hand every pane that process's `NO_COLOR` / `TERM_PROGRAM` / `OTTY_*`, so every agent CLI printed in one colour. The launcher's terminal identity is dropped before the login shell is asked for the user's environment, and panes are stamped with Fleet's own `TERM_PROGRAM` / `COLORTERM`. | Measured in a live pane before and after: `NO_COLOR=[1]` and one colour, then `NO_COLOR=[] COLORTERM=[truecolor] TERM_PROGRAM=[Fleet]` and `rgb(255,92,87)` / `rgb(90,247,142)` / `rgb(87,199,255)` / `rgb(255,128,0)`; 3 unit cases on the key predicate; [docs/otty-parity-notes.md](otty-parity-notes.md). |
| Paste parity                   | `d955a0b7`, `9174c2e9`, `47853ca2`, `987714f6`, `87c658d2`, `b972f300` | An IME commit stops printing twice, `Cmd+V` turns a clipboard picture into a temp file path in the pane, and a pane on the far side of ssh gets the picture uploaded instead of an unopenable local path. The picture is read once, written `0600` in a `0700` directory and swept after a week. Smooth scrolling was tried in the same round and removed again - xterm cancels it whenever the buffer moves, so it broke scrolling in exactly the panes people scroll ([docs/otty-parity-notes.md](otty-parity-notes.md)). | 16 cases across `composition-guard.test.ts` and `paste-image.test.ts`; `Cmd+V` observed putting `/var/folders/.../T/fleet-paste/image-1789333357637.png` on a live pane's input line with the file on disk as `-rw-------`; one notch at `smoothScrollDuration` 125 measured moving `viewportY` 0 rows while a pane printed, against 3 rows at 0; 256 test files / 3261 cases. |
...
| Pane scrolling is quantized to whole rows.                                        | `src/renderer/src/hooks/use-terminal.ts`             | Understood, not fixable here. xterm's DOM renderer has no sub-row offset, so pixel-granularity scrolling needs a renderer change (the same project as inline images).                                                                                              |
| `smoothScrollDuration` cannot be turned on to ease that.                          | `src/renderer/src/hooks/use-terminal.ts`             | Measured and rejected: xterm cancels the animation whenever the buffer moves, so an upward notch does nothing in a pane that is printing. The comment where the option was sits there to stop it being re-added. [docs/learnings/2026-09-14-xterm-smooth-scroll-dies-when-the-buffer-moves.md](learnings/2026-09-14-xterm-smooth-scroll-dies-when-the-buffer-moves.md) |
