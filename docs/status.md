...
| Paste and scroll parity        | `d955a0b7`, `9174c2e9`, `47853ca2`                                         | Three pane-level annoyances next to Otty: an IME commit no longer prints twice, `Cmd+V` turns a clipboard picture into a temp file path in the pane (plain `Ctrl+V` still sends `\x16`), and wheel scrolling eases over 125ms instead of landing in one frame. Pixel-granularity scrolling and Otty's other paste modes are still out of reach - see [docs/otty-parity-notes.md](otty-parity-notes.md). | 8 `composition-guard` cases and 2 `paste-image` cases pass; `Cmd+V` observed putting `/var/folders/.../T/fleet-paste/image-1789328612168.png` on a live pane's input line (2400x1600 PNG on disk); against the same xterm 6.0.0 build one notch stepped `viewportY` `376 → 375 → 374 → 373` over 57ms and frame pacing stayed p95 17.7ms through 20 notches. |
...
| Pane scrolling is eased but still quantized to whole rows.                        | `src/renderer/src/hooks/use-terminal.ts`             | Understood, not fixable here. xterm's DOM renderer has no sub-row offset, so pixel-granularity scrolling needs a renderer change (the same project as inline images), not an option.              |
